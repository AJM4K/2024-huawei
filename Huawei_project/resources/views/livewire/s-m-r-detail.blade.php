<div class="container mx-auto mt-8 px-4">
    ...
    <!-- SMR Information Section -->
    <div class="bg-white shadow rounded-lg p-6 border border-gray-200 mb-6">
        <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $smr->smr_code }}</h3>
        ...
        <p class="text-sm text-gray-600 mb-2">Warehouse: {{ $smr->warehouse }}</p>
        <p class="text-sm text-gray-600 mb-2">Warehouse Location: {{ $smr->warehouse_location }}</p>
        <p class="text-sm text-gray-600 mb-2">Sub Contract: {{ $smr->sub_contract }}</p>
        <p class="text-sm text-gray-600 mb-2">SMR Status: {{ $smr->smr_status }}</p>
    </div>

    <!-- SMR Items Section -->
    <div class="bg-white shadow rounded-lg p-6 border border-gray-200 mb-6">
        <h2 class="text-lg font-bold text-gray-900 mb-4">SMR Items</h2>
    ...
    <!-- Vouchers Section -->
    <div class="bg-white shadow rounded-lg p-6 border border-gray-200 mb-6">
        <h2 class="text-lg font-bold text-gray-900 mb-4">Vouchers</h2>
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-100">
                <tr>
            ...
            <tbody>
                @if (empty($vouchers))
                    <tr>
                        <td colspan="3" class="px-6 py-4 text-sm text-gray-900 text-center">There are no vouchers attached</td>
                    </tr>
                @else
