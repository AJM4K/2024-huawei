<div class="container mx-auto mt-8">
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6 border border-gray-200">
        ...
        <p>in :{{ DB::table('po_in_item')
                ->where('po_number', $poDetails->po_number )->sum('quantity_in') }}</p>
                <p>remaining in :{{ DB::table('po_in_item')
                                ->where('po_number', $poDetails->po_number )->sum('quantity_in') - DB::table('po_out_item')
                                ->where('po_number', $poDetails->po_number )->sum('quantity_delivered') }}</p>
       
                                <p>out : {{ DB::table('po_out_item')
                                    ->where('po_number', $poDetails->po_number )->sum('quantity_delivered') }}</p>
